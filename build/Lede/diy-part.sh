...
# 后台IP设置
export Ipv4_ipaddr="10.10.10.2"                                        # 修改JeremyWrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"                                    # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="JeremyWrt"                                             # 修改主机名称为JeremyWrt(填0为不作修改)
...
export Default_theme="design"                                          # 多主题时,选择design为默认第一主题 (填写主题名称,填0为不作修改)
...
export Customized_Information="_Jeremy_ $(TZ=UTC-8 date "+%Y.%m.%d")"  # 个性签名,你想写啥就写啥，(填0为不作修改)

# 更换固件内核
export Replace_Kernel="6.12"                                           # 更换内核版本,在对应源码的[target/linux/架构]查看patches-x.x,看看x.x有啥就有啥内核了(填入内核x.x版本号,填0为不作修改)
